...
groupadd rabbitmq
useradd --home-dir ~ --shell /bin/bash --comment 'rabbitmq user' -g rabbitmq --create-home rabbitmq
...
yum update -y
yum install -y epel-release iputils
yum install -y socat
...
chmod +x ~/bin/hostname-fix ~/.bashrc
